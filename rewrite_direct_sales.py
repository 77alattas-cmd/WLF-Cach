...
pattern = re.compile(
    r'(// 3\. GROUPS TAB BAR \(بدون تمرير وبشكل مجمع إذا امتلأت FlowRow\).*?)(\s*\}\s*} else \{\s*item \{\s*Box\([^)]+\)\s*\{\s*Text\([^)]+\)\s*\}\s*\}\s*\}\s*if \(numpad\.isVisible\))',
    re.DOTALL
)

replacement = """// 3. GROUPS TAB BAR OR ACCORDIONS
                ...
                if (activeGroups.isEmpty()) {
                    item {
                        Box(
                            ...
                            Text(
                                text = "لا توجد مبيعات نشطة حالياً. يمكنك تفعيل المجموعات من تبويب تنظيم.",
